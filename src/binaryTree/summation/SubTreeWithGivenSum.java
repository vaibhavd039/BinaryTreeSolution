...
public class SubTreeWithGivenSum {
    public  static Btree mynode=null;
    public static void main(String[] args) {
        ...
        getSumOfLeaftSubTree(root,15);
        System.out.println(mynode!=null? mynode.data : -1);
    }

    public static int getSumOfLeaftSubTree(Btree node, int sum){
        if(node == null)
            return 0;
        if(node.data+getSumOfLeaftSubTree(node.left, sum)+getSumOfLeaftSubTree(node.right, sum) == sum)
            mynode = node;
        return sum;
    }
